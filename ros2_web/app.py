...
def start_camera_service():
    if os.environ.get("WERKZEUG_RUN_MAIN") == "true" or not app.debug:
        try:
            subprocess.Popen([sys.executable, "camera_service.py"])
            print("✅ camera_service.py servisi arka planda başlatıldı (Port: 5001)")
        except Exception as e:
            print(f"❌ camera_service.py başlatılamadı: {e}")
